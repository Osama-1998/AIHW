...
def run_astar_for_weights_in_range(heuristic_type: HeuristicFunctionType, problem: GraphProblem, n: int = 30,
    ...
    for weight in weights_arr:
        wa = AStar(heuristic_type, weight, max_nr_states_to_expand)
        res = wa.solve_problem(problem)
        if res.is_solution_found:
            costs_list.append(res.solution_cost)
            expanded_list.append(res.nr_expanded_states)
            weights_list.append(weight)
    plot_distance_and_expanded_wrt_weight_figure(problem.name, weights_list, costs_list, expanded_list)
# --------------------------------------------------------------------
# ------------------------ StreetsMap Problem ------------------------
# --------------------------------------------------------------------
...
def map_problem_experiments():
    ...
    curr_time_map_problem = MapProblem(streets_map, start_point, target_point, 'current_time')
    uc = UniformCost()
    res = uc.solve_problem(curr_time_map_problem)
    print(res)
    ...
    res = a_star.solve_problem(curr_time_map_problem)
    print(res)
    ...
    #       solve the same `map_problem` with it and print the results (as before).
    a_star2 = AStar(TimeBasedAirDistHeuristic)
    res = a_star2.solve_problem(curr_time_map_problem)
    print(res)
    ...
    run_astar_for_weights_in_range(TimeBasedAirDistHeuristic, curr_time_map_problem)
    ...
    #                  solve the same `map_problem` with it and print the results (as before).
    curr_time_map_problem.set_additional_shortest_paths_based_data()
    a_star3 = AStar(ShortestPathsBasedHeuristic)
    res = a_star3.solve_problem(curr_time_map_problem)
    print(res)
    ...
    #                   solve the same `map_problem` with it and print the results (as before).
    curr_time_map_problem.set_additional_history_based_data()
    a_star3 = AStar(HistoryBasedHeuristic)
    res = a_star3.solve_problem(curr_time_map_problem)
    print(res)
